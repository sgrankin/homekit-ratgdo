...
extern "C" void *test_calloc(size_t n,size_t size) {
    auto p=test_malloc(n*size);if(p)std::memset(p,0,n*size);return p;
}
extern "C" void test_free(void *p) {
    if(p){assert(allocations.erase(p)==1);std::free(p);}
}
extern "C" char *test_strdup(const char *s) {
    auto p=static_cast<char *>(test_malloc(strlen(s)+1));if(p)strcpy(p,s);return p;
}
...
struct Socket {bool stopped=false;void stop(){stopped=true;}};
...
struct homekit_server_t {client_context_t *clients=nullptr;};
...
void send_client_events(client_context_t *,client_event_t *head) {
    for(auto p=head;p;p=p->next)sent.push_back({p->characteristic,p->value.int_value});
}
...
int main() {
    ...
    homekit_value_t value{};value.format=homekit_format_int;
    for(int iteration=0;iteration<1000;iteration++) {
        Queue_t queue{};assert(q_init(&queue,sizeof(characteristic_event_t *),4,FIFO,false));
        Socket socket;client_context_t client;client.socket=&socket;client.event_queue=&queue;
        homekit_server_t server;server.clients=&client;
        for(int i=0;i<100;i++) {
            value.int_value=i;
        ...
        sent.clear();homekit_server_process_notifications(&server);
        ...
        for(auto &ch:characteristics)client_notify_characteristic(&ch,value,&client);
        assert(q_getCount(&queue)==4&&socket.stopped&&client.disconnect);
        homekit_event_queue_clear(&queue);q_kill(&queue);assert(allocations.empty());
    }
    for(int failPoint : {0,1,2}) {
        Queue_t queue{};q_init(&queue,sizeof(characteristic_event_t *),4,FIFO,false);
        Socket socket;client_context_t client;client.socket=&socket;client.event_queue=&queue;
        homekit_server_t server;server.clients=&client;
        for(int i=0;i<3;i++)client_notify_characteristic(&characteristics[i],value,&client);
        failAfter=failPoint;sent.clear();homekit_server_process_notifications(&server);failAfter=-1;
        assert(client.disconnect&&socket.stopped&&sent.empty());
        assert(q_getCount(&queue)==0&&allocations.size()==1);
        q_kill(&queue);assert(allocations.empty());
    }
    Queue_t queue{};q_init(&queue,sizeof(characteristic_event_t *),4,FIFO,false);
    Socket socket;client_context_t client;client.socket=&socket;client.event_queue=&queue;
    failAfter=0;client_notify_characteristic(&characteristics[0],value,&client);failAfter=-1;
    ...
    value.format=homekit_format_string;value.string_value=const_cast<char *>("one");
    ...
    failAfter=0;assert(!homekit_event_enqueue(&queue,&characteristics[0],&value));failAfter=-1;
    characteristic_event_t *pending;assert(q_peekIdx(&queue,&pending,0));
    ...
    uint8_t data[]={1,2};tlv_t item{};item.type=1;item.size=2;item.value=data;
    tlv_values_t tlv{};tlv.head=&item;value.format=homekit_format_tlv;value.tlv_values=&tlv;
    for(int failure=0;failure<4;failure++) {
        q_init(&queue,sizeof(characteristic_event_t *),4,FIFO,false);
        failAfter=failure;assert(!homekit_event_enqueue(&queue,&characteristics[0],&value));failAfter=-1;
        assert(q_getCount(&queue)==0&&allocations.size()==1);q_kill(&queue);
    ...
    first->next=second;server.clients=first;activeServer=&server;
    homekit_server_process(&server);
    assert(processedClients==2&&allocations.empty());
    puts("HomeKit: 1000 bursts/disconnects, newest values, bounded queue, allocation failures: passed");
}
