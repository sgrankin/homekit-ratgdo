...
struct Config
{
    ...
    const char *getwwwUsername()
    {
        return "admin";
    }
    const char *getwwwCredentials()
    {
        return "";
    }
...
struct Server
{
    ...
    std::string body;
    const String &arg(const char *key)
    {
        return query[key];
    }
    size_t args()
    ...
    bool authenticateDigest(const char *, const char *)
    {
...
void reset()
{
    ...
    userConfig = &config;
    nowMs = 0;
    restarts = shutdowns = mdnsSends = socketStops = 0;
    mdnsOpen = true;
    ebootPending = false;
    suspend_service_loop = false;
...
{
    server.upload_.status = status;
    server.upload_.currentSize = bytes;
    if (status == UPLOAD_FILE_WRITE)
        server.upload_.totalSize += bytes;
    handle_firmware_upload();
}
void assertRecovery()
{
    assert(otaSession.recovering());
    assert(!ebootPending);
    announce_mdns();
    assert(mdnsSends == 0);
    poll_ota_recovery();
...
int main()
{
    ...
    assert(shutdowns == 1 && !mdnsOpen);
    event(UPLOAD_FILE_ABORTED);
    assertRecovery(); // includes abort with zero data
    ...
    event(UPLOAD_FILE_ABORTED);
    assertRecovery(); // millis wraps during deferred reboot
    ...
    event(UPLOAD_FILE_END);
    handle_update();
    assert(server.response == 200 && ebootPending);
    assert(otaSession.complete());
    nowMs = 180000;
    announce_mdns();
    poll_ota_recovery();
    assert(!mdnsSends && !restarts);
    ...
    event(UPLOAD_FILE_START);
    assert(!shutdowns && !Update.begins);

    reset();
    handle_update();
    assert(server.response == 400 && !restarts); // no file

    reset();
    event(UPLOAD_FILE_START);
    event(UPLOAD_FILE_START);
    assertRecovery(); // double shutdown rejected
    // Metadata failure does not poison a subsequent valid attempt.

    reset();
    server.query["md5"] = "bad";
    event(UPLOAD_FILE_START);
    server.query["md5"] = std::string(32, 'b');
    event(UPLOAD_FILE_START);
    assert(shutdowns == 1);
    // No stale digest inherited by a legacy uploader.

    reset();
    strcpy(firmwareMD5, std::string(32, 'a').c_str());
    server.query.erase("md5");
    event(UPLOAD_FILE_START);
    assert(firmwareMD5[0] == 0);
    ...
    assert(socketStops == 1); // parser stalled after write error
    event(UPLOAD_FILE_ABORTED);
    poll_ota_recovery();
    assert(restarts == 1);
